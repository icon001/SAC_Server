...
type
  TfmBuildingCode = class(TfmASubForm)
  ...
  private
    ...
    Function AddBuildingCode : Boolean;
    Function UpdateBuildingCode : Boolean;
  private
...
var
  fmBuildingCode: TfmBuildingCode;
...
function TfmBuildingCode.AddBuildingCode: Boolean;
var
...
begin
  ...
  stDongCode := FillZeroStrNum(Trim(ed_dongcode.Text),4);

  if Not dmDBInsert.InsertIntoTB_BUILDINGCODE_Value(stBuildingCode,ed_BuildingName.Text,stDongCode) then
  begin
...
procedure TfmBuildingCode.AdvStrinGridSetAllCheck(Sender: TObject;
  bchkState: Boolean);
var
  i : integer;
begin
...
procedure TfmBuildingCode.btn_AddClick(Sender: TObject);
begin
  inherited;
  pm_CodeAddClick(pm_CodeAdd);
end;

procedure TfmBuildingCode.btn_CancelClick(Sender: TObject);
begin
  inherited;
  Add.Visible := False;
end;

procedure TfmBuildingCode.btn_CloseClick(Sender: TObject);
begin
  inherited;
  Close;
end;

procedure TfmBuildingCode.btn_DeleteClick(Sender: TObject);
var
  i : integer;
  bChkState : Boolean;
  stMessage : string;
...
begin
  ...
  begin
    ...
    begin
      ...
      if bChkState then
      begin
        dmDBDelete.DeleteTB_BUILDINGCODE_Value(cells[1,i]);
...
procedure TfmBuildingCode.btn_minimizeClick(Sender: TObject);
begin
  inherited;
  windowState := wsNormal;
end;

procedure TfmBuildingCode.btn_SaveClick(Sender: TObject);
var
  stMessage : string;
  i,j,k : integer;
  bResult : Boolean;
begin
  inherited;
(*
  if Not isDigit(ed_DongCode.Text) then
  begin
    showmessage('동코드는 숫자타입입니다.');
    Exit;
  end;
*)
  if ed_BuildingName.Text = '' then
  begin
    showmessage('동명칭을 입력 하여 주세요.');
    Exit;
  end;
  ...
  if L_stWork = 'ADD' then bResult := AddBuildingCode
  else if L_stWork = 'UPDATE' then bResult := UpdateBuildingCode;
  ...
  Add.Visible := False;
  SearchList(ed_BuildingCode.Text,sg_List.TopRow);
  fmMain.FORMCHANGE(con_FormBuildingCode,'TRUE');
end;

procedure TfmBuildingCode.btn_SearchClick(Sender: TObject);
begin
  inherited;
  SearchList('');
end;

procedure TfmBuildingCode.btn_UpdateClick(Sender: TObject);
begin
  inherited;
  sg_ListDblClick(sg_List);
end;

procedure TfmBuildingCode.CommandArrayCommandsTFORMNAMEExecute(Command: TCommand;
  Params: TStringList);
begin
  inherited;
...
procedure TfmBuildingCode.CommandArrayCommandsTGRADEREFRESHExecute(
...
procedure TfmBuildingCode.CommandArrayCommandsTMENUIDExecute(Command: TCommand;
  Params: TStringList);
begin
  inherited;
...
procedure TfmBuildingCode.ed_BuildingNameKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
...
procedure TfmBuildingCode.ed_SearchNameChange(Sender: TObject);
begin
  inherited;
  SearchList('');
end;

procedure TfmBuildingCode.FormChangeEvent(aFormName: integer);
begin
//  case aFormName of
//  end;
end;

procedure TfmBuildingCode.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  fmMain.FORMENABLE(con_FormBuildingCode,'FALSE');
...
procedure TfmBuildingCode.FormCreate(Sender: TObject);
begin
  inherited;
...
function TfmBuildingCode.FormNameSetting: Boolean;
begin
...
procedure TfmBuildingCode.FormResize(Sender: TObject);
begin
  inherited;
...
procedure TfmBuildingCode.FormShow(Sender: TObject);
begin
  inherited;
...
//  self.FindSubForm('Main').FindCommand('FORMENABLE').Execute;
  SearchList('');
  fmMain.FORMENABLE(con_FormBuildingCode,'TRUE');
end;

procedure TfmBuildingCode.pm_CodeAddClick(Sender: TObject);
begin
  inherited;
...
procedure TfmBuildingCode.pm_UpdateCodeClick(Sender: TObject);
begin
  inherited;
  sg_ListDblClick(sg_List);
end;

function TfmBuildingCode.SearchList(aCurrentCode:string;aTopRow:integer = 0): Boolean;
var
  stSql : string;
...
begin
  ...
  stSql := dmDBSelect.SelectTB_BUILDINGCODE_Name(ed_SearchName.Text);
  L_nCheckCount := 0;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          //cells[1,nRow] := FindField('BC_DONGCODE').AsString;
...
procedure TfmBuildingCode.sg_ListCheckBoxClick(Sender: TObject; ACol,
...
procedure TfmBuildingCode.sg_ListDblClick(Sender: TObject);
var
  nIndex : integer;
begin
  inherited;
  ...
  begin
    ...
    ed_DongCode.Text := cells[1,Row];
    ed_BuildingCode.Text := cells[1,Row];
    ed_BuildingName.Text := cells[2,Row];
  end;
...
procedure TfmBuildingCode.TimerProgressTimer(Sender: TObject);
begin
  inherited;
...
function TfmBuildingCode.UpdateBuildingCode: Boolean;
var
  stBuildingCode : string;
  stDongCode : string;
  stMessage : string;
...
begin
  ...
  if Not dmDBUpdate.UpdateTB_BUILDINGCODE_BuildingName(stBuildingCode,ed_BuildingName.Text,stDongCode) then
  begin
    stMessage := stringReplace('저장 실패','$WORK',btn_Save.Caption,[rfReplaceAll]);
    Application.MessageBox(PChar(stMessage),'Error',MB_OK);
    Exit;
...
initialization
  RegisterClass(TfmBuildingCode);
Finalization
  UnRegisterClass(TfmBuildingCode);
