...
begin
  ...
  if cmb_SearchBuilding.ItemIndex > -1 then
     stBuildingCode := DeviceComBuildingList.Strings[cmb_SearchBuilding.ItemIndex];
  dmComboBoxCodeLoad.LoadDoorCode(stBuildingCode,'',DeviceComDoorCodeList,TComboBox(cmb_Node),True,'전체');
...
procedure TfmDeviceComMonitoring.FormChangeEvent(aFormName: integer);
var
  stOldData : string;
  nIndex : integer;
begin
  case aFormName of
    con_FormBuildingCode : begin
      if cmb_SearchBuilding.ItemIndex > -1 then
         stOldData := DeviceComBuildingList.Strings[cmb_SearchBuilding.ItemIndex];
      dmComboBoxCodeLoad.LoadBuildingCode(DeviceComBuildingList,TComboBox(cmb_SearchBuilding),True,'전체');
      nIndex := DeviceComBuildingList.IndexOf(stOldData);
