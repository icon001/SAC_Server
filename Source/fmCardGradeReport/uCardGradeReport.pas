...
begin
  case aFormName of
    con_FormBuildingAreaCode : begin
      if cmb_DoorBuildingArea.ItemIndex > -1 then
         stOldData := DoorBuildingAreaCodeList.Strings[cmb_DoorBuildingArea.ItemIndex];
      cmb_DoorBuildingAreaChange(cmb_DoorBuildingArea);
      nIndex := DoorBuildingAreaCodeList.IndexOf(stOldData);
      cmb_DoorBuildingArea.ItemIndex := nIndex;

      if cmb_CardBuildingArea.ItemIndex > -1 then
         stOldData := CardBuildingAreaCodeList.Strings[cmb_CardBuildingArea.ItemIndex];
      cmb_CardBuildingChange(cmb_CardBuilding);
      nIndex := CardBuildingAreaCodeList.IndexOf(stOldData);
      cmb_CardBuilding.ItemIndex := nIndex;
    end;
    con_FormBuildingCode : begin
      if cmb_DoorBuilding.ItemIndex > -1 then
         stOldData := DoorBuildingCodeList.Strings[cmb_DoorBuilding.ItemIndex];
      ...
      cmb_DoorBuilding.ItemIndex := nIndex;
      stOldData := '';
      if cmb_CardBuilding.ItemIndex > -1 then
         stOldData := CardBuildingCodeList.Strings[cmb_CardBuilding.ItemIndex];
