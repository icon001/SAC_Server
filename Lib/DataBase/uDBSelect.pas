...
type
  ...
  public
    ...
    Function SelectTB_ACCESSEVENT_AccessReport(aStartDate, aStartTime,aEndDate, aEndTime, aDoorBuildingCode, aDoorBuildingAreaCode, aDoorSeq, aCardGubunCode, aCardBuildingCode,
            aCardBuildingAreaCode, aCardNo, aCardName:string;aDateType:integer=0):string;
    ...
    Function SelectTB_CARD_Name(aCardGubun,aBuildingCode,aBuildingAreaCode,aName,aCardNo:string):string;
...
function TdmDBSelect.SelectTB_ACCESSEVENT_AccessReport(aStartDate, aStartTime,
  ...
  aCardName: string;aDateType:integer=0): string;
var
  stSql : string;
begin
  ...
  begin
    ...
    begin
      stSql := stSql + ' AND Mid(a.AE_INSERTTIME,1,14) BETWEEN ''' + aStartDate + aStartTime + ''' AND ''' + aEndDate + aEndTime + ''' ';
    end;
  end else
  begin
    stSql := ' Select a.*,d.BC_BUILDINGNAME as DoorBuildingName,e.BC_BUILDINGAREANAME as DoorBuildingAreaName, ';
    ...
    begin
      stSql := stSql + ' AND substr(a.AE_INSERTTIME,1,14) BETWEEN ''' + aStartDate + aStartTime + ''' AND ''' + aEndDate + aEndTime + ''' ';
    end;
...
function TdmDBSelect.SelectTB_CARD_Name(aCardGubun, aBuildingCode,
  aBuildingAreaCode, aName,aCardNo: string): string;
var
  stSql : string;
begin
  if G_nDBTYPE = MDB then
...
begin
  if G_nDBTYPE = MDB then
  begin
    stSql := 'select a.*,b.CA_CARDNO as CARDNO,b.CA_STARTYMDHMS,b.CA_ENDYMDHMS,b.CA_ADDR from TB_DEVICECARDNO a ';
